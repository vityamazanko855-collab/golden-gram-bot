...
RANKS=[{"name":"🟢 НОВИЧОК","need":0,"reward":0},{"name":"🔵 ИГРОК","need":10000,"reward":5000},{"name":"🟣 ОПЫТНЫЙ","need":100000,"reward":25000},{"name":"🟠 ПРОФИ","need":1000000,"reward":100000},{"name":"🔴 ЛЕГЕНДА","need":10000000,"reward":500000},{"name":"👑 КОРОЛЬ","need":50000000,"reward":2000000}]

def get_rank(b):
 for i in range(len(RANKS)-1,-1,-1):
  if b>=RANKS[i]["need"]:return RANKS[i]
 return RANKS[0]
...
FORTUNE_WHEEL=[{"name":"💀 ПРОИГРЫШ","mult":0,"prob":20},{"name":"💰 x2","mult":2,"prob":25},{"name":"💰 x3","mult":3,"prob":20},{"name":"💰 x5","mult":5,"prob":15},{"name":"💰 x10","mult":10,"prob":10},{"name":"💰 x20","mult":20,"prob":5},{"name":"💰 x50","mult":50,"prob":3},{"name":"💰 x100","mult":100,"prob":2}]
def spin_wheel():
...
def get_achievements_list(uid):
 ...
 for aid,ach in achievements_data.items():
  if aid in user_achievements[uid]:lines.append(f"✅ {ach['name']} - {ach['desc']} (+{format_amount(ach['reward'])})")
  else:lines.append(f"❌ {ach['name']} - {ach['desc']}")
 return"\n".join(lines)
...
def get_main_keyboard_page1():
 ...
 kb.add(InlineKeyboardButton("💰 Баланс",callback_data="menu_balance"),InlineKeyboardButton("📊 Профиль",callback_data="menu_profile"),InlineKeyboardButton("🎲 Рулетка",callback_data="menu_roulette"),InlineKeyboardButton("🎲 Кости",callback_data="menu_dice"),InlineKeyboardButton("🎰 Слот",callback_data="menu_slot"),InlineKeyboardButton("🎡 Колесо",callback_data="menu_wheel"),InlineKeyboardButton("➡️ ДАЛЕЕ",callback_data="menu_page2"))
 return kb

def get_main_keyboard_page2():
 kb=InlineKeyboardMarkup(row_width=2)
 kb.add(InlineKeyboardButton("💣 Мины",callback_data="menu_mines"),InlineKeyboardButton("🃏 Блэкджек",callback_data="menu_blackjack"),InlineKeyboardButton("🎁 Задания",callback_data="menu_quests"),InlineKeyboardButton("🏆 Топ",callback_data="menu_top"),InlineKeyboardButton("🏅 Ачивки",callback_data="menu_achievements"),InlineKeyboardButton("❓ Помощь",callback_data="menu_help"),InlineKeyboardButton("⬅️ НАЗАД",callback_data="menu_page1"))
 return kb
...
async def menu_cb(call):
 ...
 if act=="balance":
  ...
  await call.message.edit_text(f"👤 <b>{name}</b>\n├ 🆔 {uid}\n├ 📊 Уровень: {lvl}\n├ 🏅 Ранг: {rank['name']}\n└ 💰 {format_amount(bal)} GOLD\n\n📊 <b>Статистика</b>\n├ 🎲 Игр: {s['played']}\n├ 🏆 Побед: {s['won']}\n├ 📈 Винрейт: {wr:.1f}%\n└ 📊 Профит: {format_amount(prof)} GOLD",parse_mode="HTML",reply_markup=get_main_keyboard_page1())
 elif act=="quests":
  ...
  texts={"roulette":"🎲 <b>СТАВКИ НА РУЛЕТКУ</b>\n\n├ 100 чёрное\n├ 250 красное\n├ 500 чётное\n├ 1000 14\n├ 2000 0\n└ 5000 1-12","dice":"🎲 <b>ИГРА КОСТИ</b>\n\n├ Команда: кости 500 на 7\n├ кости 500 на дубль\n├ кости 500 на чёт\n├ кости 500 на нечёт\n├ кости 500 на больше\n└ кости 500 на меньше","slot":"🎰 <b>СЛОТ-МАШИНА</b>\n\n├ Команда: слот 100\n├ 🍒🍒🍒 = x10\n├ 🍋🍋🍋 = x8\n├ 🍊🍊🍊 = x8\n├ 🔔🔔🔔 = x20\n├ 7️⃣7️⃣7️⃣ = x50\n└ 💎💎💎 = ДЖЕКПОТ x100","wheel":"🎡 <b>КОЛЕСО ФОРТУНЫ</b>\n\n├ Команда: колесо 100\n├ Множители от x0 до x100\n└ Удачи!","mines":"💣 <b>ИГРА МИНЫ</b>\n\n├ Команда: мины 100\n├ Поле 5x5, 3 мины\n├ Каждая клетка +0.14x\n└ Максимум x4.0","blackjack":"🃏 <b>БЛЭКДЖЕК</b>\n\n├ Команда: bj 100\n├ Мин. ставка: 100\n├ Блэкджек: x2.5\n├ Победа: x2\n└ Сдача: 50%","help":"👑 <b>GOLDEN GOLD ROULETTE</b>\n\n🎲 СТАВКИ:\n├ 100 чёрное\n├ 250 красное\n├ 500 чётное\n├ 1000 14\n├ 2000 0\n└ 5000 1-12\n\n🎲 КОСТИ: кости 500 на 7\n🎰 СЛОТ: слот 100\n🎡 КОЛЕСО: колесо 100\n💣 МИНЫ: мины 100\n🃏 БЛЭКДЖЕК: bj 100\n\n📌 КОМАНДЫ:\n├ б - баланс\n├ профиль - статистика\n├ топ - топ игроков\n├ ачивки - достижения\n├ ранг - твой ранг\n├ бонус - бонус\n├ задания - задания\n├ го - запуск\n├ отмена - отмена ставок\n├ дать 500 (ответом)\n└ дать всё (ответом)"}
  if act in["roulette","dice","slot","wheel"]:await call.message.edit_text(texts.get(act,"❓ Помощь"),parse_mode="HTML",reply_markup=get_main_keyboard_page1())
  else:await call.message.edit_text(texts.get(act,"❓ Помощь"),parse_mode="HTML",reply_markup=get_main_keyboard_page2())
...
async def handle(m):
 ...
 if text.lower().startswith("кости "):
  ...
  if bet_type not in VALID_DICE_BETS:await m.reply(f"❌ Неверный тип ставки!\nВарианты: на [2-12], на дубль, на чёт, на нечёт, на больше, на меньше\nПример: кости 500 на 7");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  old_bal=bal;user_balances[uid]=bal-bet
 ...
 if text.lower().startswith("bj ") or text.lower().startswith("блекджек "):
  ...
  pv=hand_value(ph)
  if pv==21:
   win=int(bet*2.5);user_balances[uid]=user_balances.get(uid,0)+win
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
