...
def load_data():
    ...
    global user_achievements, user_prestige, user_titles, user_active_title, user_badges, user_referrals, user_referrer, used_promocodes
    try:
...
def check_badges(uid):
    if uid not in user_badges:
        user_badges[uid] = []
    stats = user_stats.get(uid, {"played": 0, "won": 0, "total_bet": 0, "total_win": 0, "mines_wins": 0, "bj_wins": 0, "slot_jackpot": 0, "max_fortune": 0, "sport_wins": 0, "rps_wins": 0, "bowling_strike": 0, "dart_bullseye": 0, "basketball_three": 0})
    current_streak = daily_quests["win_3_streak"]["current_streak"].get(uid, 0) if "win_3_streak" in daily_quests else 0
    balance = user_balances.get(uid, 0)
...
def get_titles_list(uid):
    ...
    for i, (tid, t) in enumerate(items):
        ...
        if tid in user_titles[uid]:
            ...
            lines.append(f"{prefix} ❌ {t['name']}")
    lines.append(f"└ 📌 Активный титул: {user_active_title.get(uid, 'Не выбран')}")
    return "\n".join(lines)
...
def normalize_bet(bet):
    b = bet.lower().strip()
    if b in ["к", "красное", "красный", "red", "🔴"]:
        return "красное"
    if b in ["ч", "чёрное", "черное", "чёрный", "черный", "black", "⚫"]:
        return "чёрное"
    if b in ["чёт", "чет", "чётное", "четное", "even"]:
        return "чётное"
    if b in ["неч", "нечётное", "нечетное", "odd"]:
        return "нечётное"
    if b in ["0", "зеро", "zero", "🟢"]:
        return "0"
    if b in ["1-я", "1я", "первая", "первый"]:
        return "1-я"
    if b in ["2-я", "2я", "вторая", "второй"]:
        return "2-я"
    if b in ["3-я", "3я", "третья", "третий"]:
        return "3-я"
    return b

def check_win(bet, num, color):
    b = normalize_bet(bet)
    if b == "красное" and color == "КРАСНОЕ":
        return True
    if b == "чёрное" and color == "ЧЁРНОЕ":
        return True
    if b == "чётное" and num != 0 and num % 2 == 0:
        return True
    if b == "нечётное" and num % 2 == 1:
        return True
    if b == "0" and num == 0:
        return True
    if b == "1-12" and 1 <= num <= 12:
        return True
    if b == "13-24" and 13 <= num <= 24:
        return True
    if b == "25-36" and 25 <= num <= 36:
        return True
    if b == "1-я" and num != 0 and num % 3 == 1:
        return True
    if b == "2-я" and num != 0 and num % 3 == 2:
        return True
    if b == "3-я" and num != 0 and num % 3 == 0:
        return True
    if b.isdigit():
        return int(b) == num
    if "-" in b:
...
def get_multiplier(bet):
    b = normalize_bet(bet)
    if b in ["красное", "чёрное", "чётное", "нечётное"]:
        return 2
    if b == "0" or b.isdigit():
        return 36
    if b in ["1-12", "13-24", "25-36", "1-я", "2-я", "3-я"]:
        return 3
    if "-" in b:
        try:
            p = b.split("-")
