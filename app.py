...
def rps_result(player, bot_choice):
    if player == bot_choice: return "draw"
    elif RPS_WIN[player] == bot_choice: return "win"
    else: return "lose"


def load_data():
    global user_balances, user_stats, user_levels, daily_streak, game_history
    global daily_quests, last_quest_reset, user_achievements, user_prestige
    global user_titles, user_active_title, user_badges, user_referrals, user_referrer
    try:
        with open(DATA_FILE, "r") as f:
            ...
            # FIX: Корректная загрузка множества использованных промокодов
            if "Gold2026" in PROMOCODES:
                used_raw = d.get("used_promocodes", [])
                PROMOCODES["Gold2026"]["used"] = set(used_raw) if isinstance(used_raw, list) else set()
    except Exception:
        pass
...
def get_current_streak(uid):
    """FIX: Централизованное получение стрика, безопасное при незаполненных квестах."""
    if "win_3_streak" in daily_quests:
        return daily_quests["win_3_streak"].get("current_streak", {}).get(uid, 0)
    return 0


def _default_stats():
    return {
        "played": 0, "won": 0, "total_bet": 0, "total_win": 0,
        "mines_wins": 0, "bj_wins": 0, "slot_jackpot": 0,
        "max_fortune": 0, "sport_wins": 0, "rps_wins": 0,
        "bowling_strike": 0, "dart_bullseye": 0, "basketball_three": 0
    }


def ensure_stats(uid):
    """FIX: Гарантирует наличие всех полей статистики для пользователя."""
    if uid not in user_stats:
        user_stats[uid] = _default_stats()
    else:
        defaults = _default_stats()
        for k, v in defaults.items():
            user_stats[uid].setdefault(k, v)
...
def get_achievements_list(uid):
    ...
    for i, (aid, ach) in enumerate(items):
        ...
        if aid in user_achievements[uid]:
            lines.append(f"{p} ✅ {ach['name']} (+{format_amount(ach['reward'])})")
        else:
            lines.append(f"{p} ❌ {ach['name']}")
    return "\n".join(lines)
...
def get_quests_status(uid):
    ...
    for qid, q in daily_quests.items():
        ...
        if qid == "earn_100k":
            prog = q.get("current_earn", {}).get(uid, 0)
...
def get_slot_win(s):
    return SLOT_PAYOUTS.get(tuple(s), 0)
...
def format_mines_field(field, revealed):
    return "\n".join(
        " ".join(field[i][j] if (i, j) in revealed else "❓" for j in range(5))
        for i in range(5)
    )
...
async def send_quest_notify(uid, qid, reward, msg=None):
    ...
    t = (f"🎉 <b>ЗАДАНИЕ ВЫПОЛНЕНО!</b> 🎉\n\n📋 {name}\n"
         ...
         f"💳 Баланс: {format_amount(user_balances.get(uid, 0))} GOLD")
    try:
...
async def send_achievement_notify(uid, ach, msg=None):
    t = (f"🏆 <b>ДОСТИЖЕНИЕ ПОЛУЧЕНО!</b> 🏆\n\n{ach['name']}\n{ach['desc']}\n\n"
         f"💰 Награда: +{format_amount(ach['reward'])} GOLD")
    try:
        if msg: await msg.reply(t, parse_mode="HTML")
        else: await bot.send_message(uid, t, parse_mode="HTML")
    except: pass
...
async def send_rank_up_notify(uid, rank, msg=None):
    t = (f"⭐ <b>НОВОЕ ЗВАНИЕ!</b> ⭐\n\n{rank['name']}\n\n"
         f"💰 Бонус: +{format_amount(rank['reward'])} GOLD")
    try:
        if msg: await msg.reply(t, parse_mode="HTML")
        else: await bot.send_message(uid, t, parse_mode="HTML")
    except: pass
...
async def send_prestige_notify(uid, prestige, msg=None):
    t = (f"🌟 <b>НОВЫЙ ПРЕСТИЖ!</b> 🌟\n\n{prestige['name']}\n\n"
         f"💰 Бонус: +{format_amount(prestige['reward'])} GOLD\n📈 Уровень сброшен до 0")
    try:
        if msg: await msg.reply(t, parse_mode="HTML")
        else: await bot.send_message(uid, t, parse_mode="HTML")
    except: pass
...
async def start_cmd(m: Message):
    ...
    if args and args.startswith("ref"):
        try:
            ...
            pass
    await m.reply(HELP_TEXT, parse_mode="HTML", reply_markup=get_main_keyboard_page1())


@dp.message_handler(commands=["add_grams"])
async def add_grams(m: Message):
...
async def menu_cb(call: CallbackQuery):
    ...
    # FIX: Навигация по страницам — edit_reply_markup, затем return
    if act == "page1":
    ...
    if act in ["balance", "profile", "roulette", "dice", "slot", "wheel"]:
        rm = get_main_keyboard_page1()
    elif act in ["rps", "sport", "bowling", "darts", "basketball", "referral", "promo"]:
        rm = get_main_keyboard_page2()
    else:
        rm = get_main_keyboard_page3()
    ...
    if act == "balance":
        ...
        ref = user_referrals.get(uid, 0)
        t = (f"👥 <b>РЕФЕРАЛЬНАЯ СИСТЕМА</b>\n\n├ 👥 Приглашено: {ref}\n"
             f"├ 💰 Награда за друга: +{format_amount(REFERRAL_REWARD)} GOLD\n"
             f"└ 🔗 Твоя ссылка:\n{link}")
    elif act == "promo":
        ...
        t = f"🎯 <b>ЕЖЕДНЕВНЫЕ ЗАДАНИЯ</b>{get_quests_status(uid)}\n\n⏰ Обновляются каждые 24 часа"
    elif act == "achievements":
        t = get_achievements_list(uid)
    elif act == "badges":
        t = get_badges_list(uid)
    elif act == "titles":
        t = get_titles_list(uid)
    elif act == "progress":
        rank = get_rank(user_balances.get(uid, 0))
        t = (f"📊 <b>ПРОГРЕСС</b>\n\n🎖️ Звание: {rank['name']}\n"
             f"🌟 Престиж: {user_prestige.get(uid, 0)}\n"
             f"📈 Уровень: {user_levels.get(uid, 0)}\n"
             f"💰 Баланс: {format_amount(user_balances.get(uid, 0))} GOLD")
    elif act == "prestige":
        can, info = can_prestige(uid)
        if can:
            t = (f"🌟 <b>ПРЕСТИЖ</b>\n\nТы готов к новому престижу!\n{info['name']}\n"
                 f"💰 Награда: +{format_amount(info['reward'])} GOLD\n\nНапиши престиж для подтверждения")
        else:
            t = f"🌟 <b>ПРЕСТИЖ</b>\n\n{info}"
    elif act == "top":
    ...
    try:
        await call.message.edit_text(t, parse_mode="HTML", reply_markup=rm)
    except Exception:
        pass
...
async def handle(m: Message):
    ...
    # ========== БЛЭКДЖЕК ==========
    if low.startswith("bj ") or low.startswith("блекджек "):
        if len(parts) < 2:
        ...
        if pv == 21:
            ...
            daily_quests["win_3_streak"]["current_streak"][uid] = \
                daily_quests["win_3_streak"]["current_streak"].get(uid, 0) + 1
            update_quest_progress(uid, "win_3_streak",
                                  daily_quests["win_3_streak"]["current_streak"][uid])
        ...
        await m.reply(
            ...
            f"Ваши карты: {format_cards(ph)} ({pv})\n"
            f"Карта дилера: {format_cards([dh[0]])}</code>",
            parse_mode="HTML", reply_markup=kb
        )
        return

    # ========== МИНЫ ==========
    if low.startswith("мины "):
        if len(parts) < 2:
        ...
        await m.answer(
            f"💎 {name}\n📌 Ставка: {format_amount(bet)} GOLD\n"
            f"💲 Выигрыш: x1.0 | {format_amount(bet)} GOLD\n\n{format_mines_field(field, [])}",
            reply_markup=kb
        )
        return
    ...
    # ========== РЕФЕРАЛЫ ==========
    if low in ["рефка", "referral", "рефералы"]:
        link = generate_referral_link(uid)
        ref = user_referrals.get(uid, 0)
        await m.reply(
    ...
    if low == "бонус":
        ...
        if now - ds["last"] >= 86400:
            ...
            save_data()
            await m.reply(
                f"<code>🎁 +{format_amount(bonus)} GOLD\n🔥 Стрик: {ds['streak']} дн.\n"
                f"💳 Новый баланс: {format_amount(user_balances[uid])} GOLD</code>",
                parse_mode="HTML"
            )
        else:
    ...
    if low in ["б", "баланс"]:
        await m.reply(
            f"<code>{name}\n💰 Баланс: {format_amount(user_balances.get(uid, 0))} GOLD</code>",
            parse_mode="HTML"
        )
        return
    ...
    if low in ["профиль", "profile"]:
        ...
        await m.reply(
            ...
            f"👥 Пригласил: {referrals}\n💰 {format_amount(bal)} GOLD\n\n"
            f"🎲 Игр: {s['played']}\n🏆 Побед: {s['won']}\n📈 Винрейт: {wr:.1f}%\n"
            f"📊 Профит: {format_amount(prof)} GOLD</code>",
            parse_mode="HTML"
        )
        return
    ...
    if low in ["престиж", "prestige"]:
        ...
        if can:
            ...
            if result:
                ...
                await m.reply(
                    f"🌟 Поздравляем! Ты достиг {result['name']}!\n"
                    f"💰 +{format_amount(result['reward'])} GOLD\n📈 Уровень сброшен до 0"
                )
            else:
    ...
    # ========== ДОСТИЖЕНИЯ / ЗНАЧКИ / ТИТУЛЫ / ЗАДАНИЯ ==========
    if low in ["достижения", "achievements"]:
        await m.reply(get_achievements_list(uid), parse_mode="HTML"); return
    if low in ["значки", "badges"]:
        await m.reply(get_badges_list(uid), parse_mode="HTML"); return
    if low in ["титулы", "titles"]:
        await m.reply(get_titles_list(uid), parse_mode="HTML"); return
    if low in ["задания", "quests"]:
        await m.reply(
            f"<code>🎯 ЕЖЕДНЕВНЫЕ ЗАДАНИЯ{get_quests_status(uid)}\n\n"
            f"⏰ Обновляются каждые 24 часа\n✨ Награда выдается автоматически!</code>",
            parse_mode="HTML"
        )
        return
    ...
    # ========== БОУЛИНГ ==========
    if low.startswith("боулинг "):
        if len(parts) != 2:
            return await m.reply("❌ Пример: боулинг 100")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: боулинг 100")
        ...
        bal = user_balances.get(uid, 0)
        if bet > bal:
            return await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD")
        pins = play_bowling()
        if pins == 10:
            mult = 3; result_text = "🎳 СТРАЙК! x3"
        elif pins >= 8:
            mult = 2; result_text = f"🎳 Спэр! {pins} кеглей! x2"
        else:
            mult = 0; result_text = f"🎳 Мимо! {pins} кеглей..."  # FIX: 0.5 → 0 (иначе убыток)
        ...
        user_stats[uid]["played"] += 1
        user_stats[uid]["total_bet"] += bet
        user_stats[uid]["total_win"] += win_amt
        if win_amt > 0:
            user_stats[uid]["won"] += 1
        if pins == 10:
        ...
        for badge in new_badges: await send_badge_notify(uid, badge, m)
        save_data()
        result_line = f"✅ ВЫ ВЫИГРАЛИ! +{format_amount(win_amt)}" if win_amt > 0 else f"❌ ВЫ ПРОИГРАЛИ! -{format_amount(bet)}"
        await m.reply(f"🎳 <b>БОУЛИНГ</b>\n\n{result_text}\n\n{result_line} GOLD", parse_mode="HTML")
        return

    # ========== ДАРТС ==========
    if low.startswith("дартс "):
        if len(parts) != 2:
            return await m.reply("❌ Пример: дартс 100")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: дартс 100")
    ...
    # ========== БАСКЕТБОЛ ==========
    if low.startswith("баскетбол "):
        if len(parts) != 2:
            return await m.reply("❌ Пример: баскетбол 100")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: баскетбол 100")
    ...
    # ========== КНБ ==========
    if low.startswith("кнб "):
        if len(parts) < 3:
            return await m.reply("❌ Пример: кнб 500 камень")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: кнб 500 камень")
        ...
        if choice not in RPS_CHOICES:
            return await m.reply("❌ Неверный выбор! Варианты: камень, ножницы, бумага")
        bal = user_balances.get(uid, 0)
        if bet > bal:
            return await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD")
        ...
        if result == "win":
            ...
            user_balances[uid] = bal - bet + win_amt
            user_stats[uid]["won"] += 1
            user_stats[uid]["rps_wins"] += 1
            user_stats[uid]["total_win"] += win_amt
            daily_quests["win_3_streak"]["current_streak"][uid] = \
                daily_quests["win_3_streak"]["current_streak"].get(uid, 0) + 1
            update_quest_progress(uid, "win_3_streak",
                                  daily_quests["win_3_streak"]["current_streak"][uid])
            update_quest_progress(uid, "win_5_streak",
                                  daily_quests["win_3_streak"]["current_streak"][uid])
            update_quest_earn(uid, win_amt)
            new_rank = check_rank_up(uid, old_bal, user_balances[uid])
            if new_rank: await send_rank_up_notify(uid, new_rank, m)
            await m.reply(
                ...
                f"Ты: {RPS_CHOICES[choice]} | Бот: {RPS_CHOICES[bot_choice]}\n\n"
                f"✅ ВЫ ВЫИГРАЛИ!\n💰 +{format_amount(win_amt)} GOLD (x2)",
                parse_mode="HTML"
            )
        elif result == "draw":
            user_balances[uid] = bal  # FIX: ставка возвращается (не вычитается и не прибавляется дважды)
            await m.reply(
                f"✊ <b>КАМЕНЬ-НОЖНИЦЫ-БУМАГА</b>\n\n"
                f"Ты: {RPS_CHOICES[choice]} | Бот: {RPS_CHOICES[bot_choice]}\n\n"
                f"🤝 НИЧЬЯ!\n💰 Ставка возвращена",
    ...
    # ========== СПОРТ ==========
    if low.startswith("спорт "):
        if len(parts) != 3:
            return await m.reply("❌ Пример: спорт 500 1\n1 - победа хозяев, X - ничья, 2 - победа гостей")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: спорт 500 1")
        ...
        if bet_type not in ["1", "X", "2"]:
            return await m.reply("❌ Неверный тип ставки! Используй 1, X или 2")
        bal = user_balances.get(uid, 0)
        if bet > bal:
            return await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD")
        event = get_sport_event()
        coef = event["coef1"] if bet_type == "1" else (event["coefX"] if bet_type == "X" else event["coef2"])
        ...
        if win:
            ...
            if new_rank: await send_rank_up_notify(uid, new_rank, m)
            await m.reply(
                f"⚽ <b>СПОРТИВНАЯ СТАВКА</b>\n\n{event['name']}\nСтавка: {bet_type} (x{coef})\n\n"
                f"✅ ВЫ ВЫИГРАЛИ!\n💰 +{format_amount(win_amt)} GOLD",
                parse_mode="HTML"
            )
        else:
            user_balances[uid] = bal - bet
    ...
    # ========== КОСТИ ==========
    if low.startswith("кости "):
        if len(parts) != 3:
            return await m.reply("❌ Пример: кости 500 на 7")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: кости 500 на 7")
        ...
        if bet_type not in VALID_DICE_BETS:
            return await m.reply("❌ Неверный тип ставки!")
        bal = user_balances.get(uid, 0)
        if bet > bal:
            return await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD")
        old_bal = bal
        ...
        if mult > 0:
            ...
            await m.reply(
                f"🎲 <b>КОСТИ</b>\n\n{format_dice(d1, d2)}\nСумма: {d1 + d2}\n\n"
                f"✅ ВЫ ВЫИГРАЛИ!\n💰 +{format_amount(win_amt)} GOLD (x{mult})",
                parse_mode="HTML"
            )
        else:
            daily_quests["win_3_streak"]["current_streak"][uid] = 0
    ...
    # ========== СЛОТ ==========
    if low.startswith("слот "):
        if len(parts) != 2:
            return await m.reply("❌ Пример: слот 100")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: слот 100")
    ...
    # ========== КОЛЕСО ==========
    if low.startswith("колесо "):
        if len(parts) != 2:
            return await m.reply("❌ Пример: колесо 100")
        try:
            bet = int(parts[1])
        except ValueError:
            return await m.reply("❌ Пример: колесо 100")
    ...
    if len(parts) >= 2:
        if game_in_progress:
            return await m.reply("⏳ Идёт игра, подождите...")
        try:
            amt = int(parts[0])
        except ValueError:
            return
    ...
    if low == "го":
        ...
        if now - last_game_time < GAME_COOLDOWN:
            return await m.reply(f"⏰ Подожди {GAME_COOLDOWN - (now - last_game_time)} сек")
        if not pending_bets:
            return await m.reply("❌ Нет активных ставок")
        ...
        try:
            ...
            for b in pending_bets:
                ...
                bid_uid = b["user_id"]
                ensure_stats(bid_uid)
                user_stats[bid_uid]["played"] += 1
                user_stats[bid_uid]["total_bet"] += amt
                user_levels[bid_uid] = user_levels.get(bid_uid, 0) + 1
                update_quest_progress(bid_uid, "play_3_games")
                update_quest_progress(bid_uid, "play_10_games")
                update_quest_progress(bid_uid, "make_bet")
                update_quest_progress(bid_uid, "make_5_bets")
                if check_win(raw, num, color):
                    mult = get_multiplier(raw)
                    win_amt = amt * mult
                    old_bal_bet = user_balances.get(bid_uid, 0)
                    user_balances[bid_uid] = user_balances.get(bid_uid, 0) + win_amt
                    user_stats[bid_uid]["won"] += 1
                    user_stats[bid_uid]["total_win"] += win_amt
                    new_rank = check_rank_up(bid_uid, old_bal_bet, user_balances[bid_uid])
                    if new_rank: await send_rank_up_notify(bid_uid, new_rank, m)
                    daily_quests["win_3_streak"]["current_streak"][bid_uid] = \
                        daily_quests["win_3_streak"]["current_streak"].get(bid_uid, 0) + 1
                    update_quest_progress(bid_uid, "win_3_streak",
                                          daily_quests["win_3_streak"]["current_streak"][bid_uid])
                    update_quest_progress(bid_uid, "win_5_streak",
                                          daily_quests["win_3_streak"]["current_streak"][bid_uid])
                    update_quest_earn(bid_uid, win_amt)
                    wins.append(f"{uname} выиграл {format_amount(win_amt)} GOLD на {raw}")
                else:
                    daily_quests["win_3_streak"]["current_streak"][bid_uid] = 0
                new_achs = check_achievements(bid_uid)
                for ach in new_achs: await send_achievement_notify(bid_uid, ach, m)
                new_titles = check_titles(bid_uid)
                for title in new_titles: await send_title_notify(bid_uid, title, m)
                new_badges = check_badges(bid_uid)
                for badge in new_badges: await send_badge_notify(bid_uid, badge, m)
            save_data()
            ...
            logging.error(f"Ошибка рулетки: {e}")
    ...
    # ========== ПЕРЕВОДЫ ==========
    if m.reply_to_message and low == "дать всё":
        tid = m.reply_to_message.from_user.id
        if uid == tid:
            return await m.reply("❌ Нельзя перевести самому себе")
        bal = user_balances.get(uid, 0)
        if bal <= 0:
            return await m.reply("❌ Нет GOLD для перевода")
        user_balances[uid] = 0
    ...
    if m.reply_to_message and low.startswith("дать "):
        try:
            amt = int(parts[1])
            assert amt > 0
        except Exception:
...
async def bj_cb(call: CallbackQuery):
    ...
    if action == "hit":
        ...
        await call.message.edit_text(
            ...
            f"Ваши карты: {format_cards(g['player_hand'])} ({pv})</code>",
            parse_mode="HTML", reply_markup=kb
        )
        ...
        del blackjack_games[uid]
        save_data()
        win_line = f"+{format_amount(win)}" if win > 0 else "0"
        await call.message.edit_text(
            f"<code>🃏 ИГРА ОКОНЧЕНА\n\nВаши карты: {format_cards(g['player_hand'])} ({pv})\n"
            f"Карты дилера: {format_cards(g['dealer_hand'])} ({dv})\n"
            f"💰 {win_line} GOLD</code>",
            parse_mode="HTML"
        )
        ...
        save_data()
        await call.message.edit_text(
            f"<code>🏳️ ВЫ СДАЛИСЬ\n💰 Возвращено {format_amount(refund)} GOLD</code>",
            parse_mode="HTML"
        )


@dp.callback_query_handler(lambda c: c.data.startswith("m_"))
async def mine_cb(call: CallbackQuery):
    await call.answer()
    uid = call.from_user.id
    ...
    await call.message.edit_text(
        ...
        f"💲 Выигрыш: x{g['multiplier']:.2f} | {format_amount(pot)} GOLD\n\n"
        f"{format_mines_field(g['field'], g['revealed'])}",
        reply_markup=get_mines_keyboard(g["field"], g["revealed"])
    )


@dp.callback_query_handler(lambda c: c.data == "cash")
async def mine_cash_cb(call: CallbackQuery):
    await call.answer()
    uid = call.from_user.id
    if uid not in mines_games:
        return
