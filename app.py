...
API_TOKEN=os.environ.get("BOT_TOKEN","8655886367:AAGQMnYq2OEGI50vn2Z1TWe1P--zp-zydr0")
...
def play_bowling():return random.randint(0,10)
def throw_dart():
 r=random.randint(1,100)
 if r<=5:return"bullseye"
 elif r<=20:return"inner"
 elif r<=50:return"outer"
 else:return"miss"
DART_MULT={"bullseye":10,"inner":5,"outer":2,"miss":0}
def shoot_ball():
 r=random.randint(1,100)
 if r<=10:return"three_point"
 elif r<=30:return"two_point"
 elif r<=60:return"free_throw"
 else:return"miss"
BASKET_MULT={"three_point":3,"two_point":2,"free_throw":1,"miss":0}

REFERRAL_REWARD=30000
def generate_referral_link(uid):return f"https://t.me/Golden_Gram_Roulette_Bot?start=ref{uid}"
...
PROMOCODES={"Gold2026":{"reward":100000,"used":set()}}
def use_promocode(uid,code):
 if code in PROMOCODES:
  if uid in PROMOCODES[code]["used"]:return None,"already_used"
...
def get_rank(balance):
 for i in range(len(RANKS)-1,-1,-1):
  if balance>=RANKS[i]["need"]:return RANKS[i]
 return RANKS[0]
def check_rank_up(uid,old_b,new_b):
 old=get_rank(old_b);new=get_rank(new_b)
 if old["name"]!=new["name"]:
  user_balances[uid]=user_balances.get(uid,0)+new["reward"]
  return new
 return None
def can_prestige(uid):
 level=user_levels.get(uid,0);current=user_prestige.get(uid,0)
 if current>=len(PRESTIGE_LEVELS):return False,"❌ Ты достиг максимального престижа!"
 next_p=PRESTIGE_LEVELS[current]
 if level>=next_p["need_level"]:return True,next_p
 return False,f"📊 Нужно {next_p['need_level']} уровень"
def do_prestige(uid):
 if uid not in user_prestige:user_prestige[uid]=0
 current=user_prestige[uid]
 if current>=len(PRESTIGE_LEVELS):return None
 next_p=PRESTIGE_LEVELS[current]
 if user_levels.get(uid,0)>=next_p["need_level"]:
  user_prestige[uid]=current+1
  user_balances[uid]=user_balances.get(uid,0)+next_p["reward"]
  user_levels[uid]=0
  return next_p
 return None
def get_sport_event():return random.choice(SPORTS_EVENTS)
def rps_result(player,bot):
 if player==bot:return"draw"
 elif RPS_WIN[player]==bot:return"win"
 else:return"lose"

def load_data():
 global user_balances,user_stats,user_levels,daily_streak,game_history,daily_quests,last_quest_reset,user_achievements,user_prestige,user_titles,user_active_title,user_badges,user_referrals,user_referrer,used_promocodes
 try:
...
def save_data():
 with open(DATA_FILE,"w")as f:
  ...
  json.dump({"user_balances":user_balances,"user_stats":user_stats,"user_levels":user_levels,"daily_streak":daily_streak,"game_history":game_history[-50:],"daily_quests":daily_quests,"last_quest_reset":last_quest_reset,"user_achievements":user_achievements,"user_prestige":user_prestige,"user_titles":user_titles,"user_active_title":user_active_title,"user_badges":user_badges,"user_referrals":user_referrals,"user_referrer":user_referrer,"used_promocodes":used_list},f,ensure_ascii=False,indent=2)

def check_achievements(uid):
 if uid not in user_achievements:user_achievements[uid]=[]
 s=user_stats.get(uid,{"played":0,"won":0,"total_bet":0,"total_win":0,"mines_wins":0,"bj_wins":0,"slot_jackpot":0,"max_fortune":0,"sport_wins":0,"rps_wins":0})
 streak=daily_quests["win_3_streak"]["current_streak"].get(uid,0)if"win_3_streak"in daily_quests else 0
 b=user_balances.get(uid,0);new=[]
 for aid,ach in achievements_data.items():
  if aid in user_achievements[uid]:continue
  earned=False
  if"need_balance"in ach and b>=ach["need_balance"]:earned=True
  elif"need_games"in ach and s.get("played",0)>=ach["need_games"]:earned=True
  elif"need_wins"in ach and s.get("won",0)>=ach["need_wins"]:earned=True
  elif"need_mines_wins"in ach and s.get("mines_wins",0)>=ach["need_mines_wins"]:earned=True
  elif"need_bj_wins"in ach and s.get("bj_wins",0)>=ach["need_bj_wins"]:earned=True
  elif"need_streak"in ach and streak>=ach["need_streak"]:earned=True
  elif"need_jackpot"in ach and s.get("slot_jackpot",0)>=ach["need_jackpot"]:earned=True
  elif"need_fortune"in ach and s.get("max_fortune",0)>=ach["need_fortune"]:earned=True
  elif"need_sport"in ach and s.get("sport_wins",0)>=ach["need_sport"]:earned=True
  elif"need_rps"in ach and s.get("rps_wins",0)>=ach["need_rps"]:earned=True
  if earned:
   user_achievements[uid].append(aid)
   user_balances[uid]=user_balances.get(uid,0)+ach["reward"]
   new.append(ach);save_data()
 return new
def check_badges(uid):
 if uid not in user_badges:user_badges[uid]=[]
 s=user_stats.get(uid,{"played":0,"won":0,"total_bet":0,"total_win":0,"mines_wins":0,"bj_wins":0,"slot_jackpot":0,"max_fortune":0,"sport_wins":0,"rps_wins":0,"bowling_strike":0,"dart_bullseye":0,"basketball_three":0})
 streak=daily_quests["win_3_streak"]["current_streak"].get(uid,0)if"win_3_streak"in daily_quests else 0
 b=user_balances.get(uid,0);ref=user_referrals.get(uid,0);new=[]
 for bid,badge in BADGES.items():
  if bid in user_badges[uid]:continue
  earned=False
  if bid=="beginner"and s.get("played",0)>=1:earned=True
  elif bid=="winner"and s.get("won",0)>=10:earned=True
  elif bid=="expert"and s.get("won",0)>=100:earned=True
  elif bid=="millionaire"and b>=1000000:earned=True
  elif bid=="billionaire"and b>=1000000000:earned=True
  elif bid=="lucky"and streak>=3:earned=True
  elif bid=="gambler"and s.get("played",0)>=500:earned=True
  elif bid=="slot_king"and s.get("slot_jackpot",0)>=1:earned=True
  elif bid=="card_master"and s.get("bj_wins",0)>=50:earned=True
  elif bid=="sapper"and s.get("mines_wins",0)>=25:earned=True
  elif bid=="fortune"and s.get("max_fortune",0)>=100:earned=True
  elif bid=="sport"and s.get("sport_wins",0)>=1:earned=True
  elif bid=="rps"and s.get("rps_wins",0)>=10:earned=True
  elif bid=="streak10"and streak>=10:earned=True
  elif bid=="bowler"and s.get("bowling_strike",0)>=1:earned=True
  elif bid=="dart_master"and s.get("dart_bullseye",0)>=1:earned=True
  elif bid=="basketball_star"and s.get("basketball_three",0)>=1:earned=True
  elif bid=="referrer"and ref>=10:earned=True
  if earned:
   user_badges[uid].append(bid)
   user_balances[uid]=user_balances.get(uid,0)+50000
   new.append(badge);save_data()
 return new
def check_titles(uid):
 if uid not in user_titles:user_titles[uid]=[]
 s=user_stats.get(uid,{"played":0,"won":0,"total_bet":0,"total_win":0,"mines_wins":0,"bj_wins":0,"slot_jackpot":0,"max_fortune":0})
 streak=daily_quests["win_3_streak"]["current_streak"].get(uid,0)if"win_3_streak"in daily_quests else 0
 b=user_balances.get(uid,0);new=[]
 for tid,t in TITLES.items():
  if tid in user_titles[uid]:continue
  earned=False
  if"need_balance"in t and b>=t["need_balance"]:earned=True
  elif"need_games"in t and s.get("played",0)>=t["need_games"]:earned=True
  elif"need_wins"in t and s.get("won",0)>=t["need_wins"]:earned=True
  elif"need_mines_wins"in t and s.get("mines_wins",0)>=t["need_mines_wins"]:earned=True
  elif"need_bj_wins"in t and s.get("bj_wins",0)>=t["need_bj_wins"]:earned=True
  elif"need_streak"in t and streak>=t["need_streak"]:earned=True
  elif"need_jackpot"in t and s.get("slot_jackpot",0)>=t["need_jackpot"]:earned=True
  elif"need_fortune"in t and s.get("max_fortune",0)>=t["need_fortune"]:earned=True
  if earned:
   user_titles[uid].append(tid)
   new.append(t);save_data()
 return new
...
def get_achievements_list(uid):
 ...
 for i,(aid,ach)in enumerate(items):
  ...
  if aid in user_achievements[uid]:lines.append(f"{p} ✅ {ach['name']} (+{format_amount(ach['reward'])})")
  else:lines.append(f"{p} ❌ {ach['name']}")
 return"\n".join(lines)
def get_badges_list(uid):
 ...
 for i,(bid,badge)in enumerate(items):
  p="└"if i==len(items)-1 else"├"
  if bid in user_badges[uid]:lines.append(f"{p} ✅ {badge['name']}")
  else:lines.append(f"{p} ❌ {badge['name']}")
 return"\n".join(lines)
def get_titles_list(uid):
 ...
 for i,(tid,t)in enumerate(items):
  p="└"if i==len(items)-1 else"├"
  if tid in user_titles[uid]:lines.append(f"{p} ✅ {t['name']}")
  else:lines.append(f"{p} ❌ {t['name']}")
...
def check_quests_reset():
 ...
 if int(time.time())-last_quest_reset>=86400:
  for q in daily_quests:
   ...
   if q in["win_3_streak","win_5_streak"]:daily_quests[q]["current_streak"]={}
   if q=="earn_100k":daily_quests[q]["current_earn"]={}
  last_quest_reset=int(time.time())
  save_data()
  return True
 return False
def update_quest_progress(uid,qid,inc=1):
 ...
 check_quests_reset()
 if qid not in daily_quests or uid in daily_quests[qid].get("completed",{}):return 0
...
def update_quest_earn(uid,amount):
 if"earn_100k"not in daily_quests or uid in daily_quests["earn_100k"].get("completed",{}):return 0
...
def get_quests_status(uid):
 ...
 for qid,q in daily_quests.items():
  ...
  if qid=="earn_100k":prog=q["current_earn"].get(uid,0)
  if uid in q.get("completed",{}):lines.append(f"\n  └ {q['name']}: ✅ ВЫПОЛНЕНО (+{format_amount(q['reward'])} GOLD)")
...
def get_dice_multiplier(t,d1,d2):
 ttl=d1+d2
 if t=="дубль":return 10 if d1==d2 else 0
 if t=="чёт":return 2 if ttl%2==0 else 0
 if t=="нечёт":return 2 if ttl%2==1 else 0
 if t=="больше":return 2 if ttl>7 else 0
 if t=="меньше":return 2 if ttl<7 else 0
 if t.isdigit():return 8 if ttl==int(t)else 0
 return 0
...
def spin_slot():return[random.choice(SLOT_SYMBOLS)for _ in range(3)]
def get_slot_win(s):return SLOT_PAYOUTS.get(tuple(s),0)
def format_slot(s):return" | ".join(s)
...
def generate_deck():deck=[(r,s)for r in RANKS_CARDS for s in SUITS];random.shuffle(deck);return deck
def card_value(card):r=card[0];return 10 if r in["J","Q","K"]else(11 if r=="A"else int(r))
def hand_value(hand):
 v=sum(card_value(c)for c in hand)
 aces=sum(1 for c in hand if c[0]=="A")
 while v>21 and aces>0:v-=10;aces-=1
 return v
def format_cards(hand):return" ".join(f"{r}{s}"for r,s in hand)if hand else"пусто"

async def send_quest_notify(uid,qid,reward,msg=None):
 name=daily_quests[qid]["name"]
 t=f"🎉 <b>ЗАДАНИЕ ВЫПОЛНЕНО!</b> 🎉\n\n📋 {name}\n💰 Награда: +{format_amount(reward)} GOLD\n\n💳 Баланс: {format_amount(user_balances.get(uid,0))} GOLD"
 try:
  if msg:await msg.reply(t,parse_mode="HTML")
  else:await bot.send_message(uid,t,parse_mode="HTML")
 except:pass
async def send_achievement_notify(uid,ach,msg=None):
 t=f"🏆 <b>ДОСТИЖЕНИЕ ПОЛУЧЕНО!</b> 🏆\n\n{ach['name']}\n{ach['desc']}\n\n💰 Награда: +{format_amount(ach['reward'])} GOLD"
 try:
  if msg:await msg.reply(t,parse_mode="HTML")
  else:await bot.send_message(uid,t,parse_mode="HTML")
 except:pass
async def send_badge_notify(uid,badge,msg=None):
 t=f"🎖️ <b>НОВЫЙ ЗНАЧОК!</b> 🎖️\n\n{badge['icon']} {badge['name']}\n{badge['desc']}\n\n💰 Награда: +50 000 GOLD"
 try:
  if msg:await msg.reply(t,parse_mode="HTML")
  else:await bot.send_message(uid,t,parse_mode="HTML")
 except:pass
...
async def menu_cb(call):
 await call.answer()
 uid=call.from_user.id;act=call.data[5:]
 if act=="page1":
  try:await call.message.edit_reply_markup(reply_markup=get_main_keyboard_page1())
  except:pass;return
 if act=="page2":
  try:await call.message.edit_reply_markup(reply_markup=get_main_keyboard_page2())
  except:pass;return
 if act=="page3":
  try:await call.message.edit_reply_markup(reply_markup=get_main_keyboard_page3())
  except:pass;return
 if act in["balance","profile","roulette","dice","slot","wheel"]:rm=get_main_keyboard_page1()
 elif act in["rps","sport","bowling","darts","basketball","referral","promo"]:rm=get_main_keyboard_page2()
 else:rm=get_main_keyboard_page3()
 if act=="balance":t=f"💰 <b>Ваш баланс</b>\n\n└ {format_amount(user_balances.get(uid,0))} GOLD"
 elif act=="profile":
  name=call.from_user.full_name;bal=user_balances.get(uid,0)
  s=user_stats.get(uid,{"played":0,"won":0,"total_bet":0,"total_win":0})
  lvl=user_levels.get(uid,0);rank=get_rank(bal);prestige=user_prestige.get(uid,0)
  active_title=user_active_title.get(uid,"Не выбран");badges_count=len(user_badges.get(uid,[]))
 ...
 elif act=="roulette":t="🎲 <b>СТАВКИ НА РУЛЕТКУ</b>\n\n├ 100 чёрное\n├ 250 красное\n├ 500 чётное\n├ 1000 14\n├ 2000 0\n└ 5000 1-12"
 elif act=="dice":t="🎲 <b>ИГРА КОСТИ</b>\n\n├ Команда: кости 500 на 7\n├ кости 500 на дубль\n├ кости 500 на чёт\n├ кости 500 на нечёт\n├ кости 500 на больше\n└ кости 500 на меньше"
 elif act=="slot":t="🎰 <b>СЛОТ-МАШИНА</b>\n\n├ Команда: слот 100\n├ 🍒🍒🍒 = x10\n├ 🍋🍋🍋 = x8\n├ 🍊🍊🍊 = x8\n├ 🔔🔔🔔 = x20\n├ 7️⃣7️⃣7️⃣ = x50\n└ 💎💎💎 = ДЖЕКПОТ x100"
 elif act=="wheel":t="🎡 <b>КОЛЕСО ФОРТУНЫ</b>\n\n├ Команда: колесо 100\n├ Множители от x0 до x100\n└ Удачи!"
 elif act=="rps":t="✊ <b>КАМЕНЬ-НОЖНИЦЫ-БУМАГА</b>\n\n├ Команда: кнб 500 камень\n├ кнб @username 500 камень\n└ Победа x2, ничья возврат"
 elif act=="sport":t="⚽ <b>СПОРТИВНЫЕ СТАВКИ</b>\n\n├ Команда: спорт 500 1\n├ 1 - победа хозяев\n├ X - ничья\n└ 2 - победа гостей"
 elif act=="bowling":t="🎳 <b>БОУЛИНГ</b>\n\n├ Команда: боулинг 100\n├ Страйк (10 кегль) = x3\n├ Спэр (8-9) = x2\n└ Мимо (0-7) = x0.5"
 elif act=="darts":t="🎯 <b>ДАРТС</b>\n\n├ Команда: дартс 100\n├ Яблочко (5%) = x10\n├ Внутреннее кольцо (15%) = x5\n├ Внешнее кольцо (30%) = x2\n└ Мимо (50%) = x0"
 elif act=="basketball":t="🏀 <b>БАСКЕТБОЛ</b>\n\n├ Команда: баскетбол 100\n├ Трёхочковый (10%) = x3\n├ Двухочковый (20%) = x2\n├ Штрафной (30%) = x1\n└ Мимо (40%) = x0"
 elif act=="referral":link=generate_referral_link(uid);ref=user_referrals.get(uid,0);t=f"👥 <b>РЕФЕРАЛЬНАЯ СИСТЕМА</b>\n\n├ 👥 Приглашено: {ref}\n├ 💰 Награда за друга: +{format_amount(REFERRAL_REWARD)} GOLD\n└ 🔗 Твоя ссылка:\n{link}"
 elif act=="promo":t="🎫 <b>ПРОМОКОДЫ</b>\n\n├ 🏷️ Gold2026 - 100 000 GOLD\n└ 📝 Введи: промокод Gold2026"
 elif act=="mines":t="💣 <b>ИГРА МИНЫ</b>\n\n├ Команда: мины 100\n├ Поле 5x5, 3 мины\n├ Каждая клетка +0.14x\n└ Максимум x4.0"
 elif act=="blackjack":t="🃏 <b>БЛЭКДЖЕК</b>\n\n├ Команда: bj 100\n├ Мин. ставка: 100\n├ Блэкджек: x2.5\n├ Победа: x2\n└ Сдача: 50%"
 elif act=="quests":t=f"🎯 <b>ЕЖЕДНЕВНЫЕ ЗАДАНИЯ</b>{get_quests_status(uid)}\n\n⏰ Обновляются каждые 24 часа\n✨ Награда выдается автоматически!"
 elif act=="achievements":t=get_achievements_list(uid)
 elif act=="badges":t=get_badges_list(uid)
 elif act=="titles":t=get_titles_list(uid)
 elif act=="progress":rank=get_rank(user_balances.get(uid,0));t=f"📊 <b>ПРОГРЕСС</b>\n\n🎖️ Звание: {rank['name']}\n🌟 Престиж: {user_prestige.get(uid,0)}\n📈 Уровень: {user_levels.get(uid,0)}\n💰 Баланс: {format_amount(user_balances.get(uid,0))} GOLD"
 elif act=="prestige":
  can,info=can_prestige(uid)
  if can:t=f"🌟 <b>ПРЕСТИЖ</b>\n\nТы готов к новому престижу!\n{info['name']}\n💰 Награда: +{format_amount(info['reward'])} GOLD\n\nНапиши /престиж для подтверждения"
  else:t=f"🌟 <b>ПРЕСТИЖ</b>\n\n{info}"
 elif act=="top":
  if not user_balances:t="📊 Пусто"
  else:
   ...
   t=txt
 else:t="👑 <b>GOLDEN GOLD ROULETTE</b>\n\n🎲 ИГРЫ:\n├ Рулетка: 100 чёрное\n├ Кости: кости 500 на 7\n├ Слот: слот 100\n├ Колесо: колесо 100\n├ КНБ: кнб 500 камень\n├ Спорт: спорт 500 1\n├ Мины: мины 100\n├ Блэкджек: bj 100\n├ Боулинг: боулинг 100\n├ Дартс: дартс 100\n├ Баскетбол: баскетбол 100\n\n📌 КОМАНДЫ:\n├ б - баланс\n├ профиль - статистика\n├ топ - топ игроков\n├ достижения - список достижений\n├ значки - список значков\n├ титулы - список титулов\n├ прогресс - прогресс\n├ престиж - престиж\n├ ранг - звание\n├ бонус - бонус\n├ задания - задания\n├ рефка - реферальная ссылка\n├ промокод Gold2026 - 100 000 GOLD\n├ го - запуск рулетки\n├ отмена - отмена ставок\n├ дать 500 (ответом)\n└ дать всё (ответом)"
 try:await call.message.edit_text(t,parse_mode="HTML",reply_markup=rm)
 except:pass

@dp.message_handler()
async def handle(m):
 global pending_bets,game_in_progress,last_game_time,game_history
 if game_in_progress and time.time()-last_game_time>60:force_reset_game();await m.reply("⚠️ Предыдущая игра принудительно завершена")
 uid=m.from_user.id;name=m.from_user.full_name;text=m.text.strip()
 if text.lower().startswith('/'):return
 parts=text.split()
 
 # БЛЭКДЖЕК (ДОЛЖЕН БЫТЬ ПЕРВЫМ!)
 if text.lower().startswith("bj ") or text.lower().startswith("блекджек "):
  ...
  if pv==21:
   ...
   for badge in new_badges:await send_badge_notify(uid,badge,m)
 ...
 if text.lower().startswith("промокод "):
  ...
  elif status=="already_used":await m.reply("❌ Ты уже использовал этот промокод!")
  else:await m.reply("❌ Неверный промокод!")
  return
 
 # РЕФЕРАЛЫ
 if text.lower() in["рефка","referral","рефералы"]:
  link=generate_referral_link(uid);ref=user_referrals.get(uid,0)
  await m.reply(f"👥 <b>РЕФЕРАЛЬНАЯ СИСТЕМА</b>\n\n├ 👥 Приглашено: {ref}\n├ 💰 Награда за друга: +{format_amount(REFERRAL_REWARD)} GOLD\n└ 🔗 Твоя ссылка:\n{link}",parse_mode="HTML")
  return
 
 # БОНУС
 if text.lower()=="бонус":
  now=int(time.time());ds=daily_streak.get(uid,{"last":0,"streak":0})
  if now-ds["last"]>=86400:
   if now-ds["last"]>=172800:ds["streak"]=0
   ds["streak"]+=1;bonus=50000
   user_balances[uid]=user_balances.get(uid,0)+bonus
   ds["last"]=now;daily_streak[uid]=ds
   save_data()
   await m.reply(f"<code>🎁 +{format_amount(bonus)} GOLD\n🔥 Стрик: {ds['streak']} дн.\n💳 Новый баланс: {format_amount(user_balances[uid])} GOLD</code>",parse_mode="HTML")
  else:rem=86400-(now-ds["last"]);await m.reply(f"<code>⏰ Через {rem//3600} ч {(rem%3600)//60} мин</code>",parse_mode="HTML")
  return
 ...
 if text.lower() in["профиль","profile"]:
  bal=user_balances.get(uid,0);s=user_stats.get(uid,{"played":0,"won":0,"total_bet":0,"total_win":0})
  lvl=user_levels.get(uid,0);rank=get_rank(bal);prestige=user_prestige.get(uid,0)
  active_title=user_active_title.get(uid,"Не выбран");badges_count=len(user_badges.get(uid,[]))
  referrals=user_referrals.get(uid,0)
  wr=(s["won"]/s["played"]*100)if s["played"]>0 else 0
 ...
 if text.lower() in["ранг","rank"]:
  bal=user_balances.get(uid,0);rank=get_rank(bal);next_rank=None
  for i,r in enumerate(RANKS):
   if r["name"]==rank["name"] and i+1<len(RANKS):next_rank=RANKS[i+1];break
  if next_rank:need=next_rank["need"]-bal;await m.reply(f"🎖️ <b>ТВОЁ ЗВАНИЕ</b>\n\n{rank['name']}\n\n📊 До звания {next_rank['name']}:\n└ {format_amount(need)} GOLD",parse_mode="HTML")
  else:await m.reply(f"🎖️ <b>ТВОЁ ЗВАНИЕ</b>\n\n{rank['name']}\n\n👑 Ты достиг максимального звания!",parse_mode="HTML")
  return
 ...
 if text.lower() in["престиж","prestige"]:
  ...
  if can:
   ...
   else:await m.reply("❌ Ошибка при повышении престижа")
  else:await m.reply(f"❌ {info}")
  return
 ...
 if text.lower().startswith("боулинг "):
  if len(parts)!=2:await m.reply("❌ Пример: боулинг 100");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: боулинг 100");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  pins=play_bowling()
  if pins==10:mult=3;result_text="🎳 СТРАЙК! x3"
  elif pins>=8:mult=2;result_text=f"🎳 Спэр! {pins} кеглей! x2"
  else:mult=0.5;result_text=f"🎳 Мимо! {pins} кеглей... x0.5"
  win_amt=int(bet*mult);old_bal=bal
  if mult>=1:user_balances[uid]=bal-bet+win_amt
  else:user_balances[uid]=bal-bet;win_amt=0
  ...
  user_stats[uid]["played"]=user_stats[uid].get("played",0)+1
  if win_amt>0:user_stats[uid]["won"]=user_stats[uid].get("won",0)+1
  user_stats[uid]["total_bet"]=user_stats[uid].get("total_bet",0)+bet
  user_stats[uid]["total_win"]=user_stats[uid].get("total_win",0)+win_amt
  user_levels[uid]=user_levels.get(uid,0)+1
  update_quest_progress(uid,"play_3_games");update_quest_progress(uid,"play_10_games")
  update_quest_progress(uid,"make_bet");update_quest_progress(uid,"make_5_bets")
  new_achs=check_achievements(uid)
  for ach in new_achs:await send_achievement_notify(uid,ach,m)
  new_badges=check_badges(uid)
  for badge in new_badges:await send_badge_notify(uid,badge,m)
 ...
 if text.lower().startswith("дартс "):
  if len(parts)!=2:await m.reply("❌ Пример: дартс 100");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: дартс 100");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  result=throw_dart();mult=DART_MULT[result];win_amt=int(bet*mult);old_bal=bal
  if mult>0:user_balances[uid]=bal-bet+win_amt
  else:user_balances[uid]=bal-bet;win_amt=0
  if result=="bullseye":result_text="🎯 ЯБЛОЧКО! x10"
  elif result=="inner":result_text="🎯 Внутреннее кольцо! x5"
  elif result=="outer":result_text="🎯 Внешнее кольцо! x2"
  else:result_text="🎯 Мимо! x0"
  new_rank=check_rank_up(uid,old_bal,user_balances[uid])
  if new_rank:await send_rank_up_notify(uid,new_rank,m)
 ...
 if text.lower().startswith("баскетбол "):
  if len(parts)!=2:await m.reply("❌ Пример: баскетбол 100");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: баскетбол 100");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  result=shoot_ball();mult=BASKET_MULT[result];win_amt=int(bet*mult);old_bal=bal
  if mult>0:user_balances[uid]=bal-bet+win_amt
  else:user_balances[uid]=bal-bet;win_amt=0
  if result=="three_point":result_text="🏀 ТРЁХОЧКОВЫЙ! x3"
  elif result=="two_point":result_text="🏀 Двухочковый! x2"
  elif result=="free_throw":result_text="🏀 Штрафной! x1"
  else:result_text="🏀 Мимо! x0"
  new_rank=check_rank_up(uid,old_bal,user_balances[uid])
  if new_rank:await send_rank_up_notify(uid,new_rank,m)
 ...
 if text.lower().startswith("кнб "):
  if len(parts)<3:await m.reply("❌ Пример: кнб 500 камень");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: кнб 500 камень");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  choice=parts[2].lower()
  if choice not in RPS_CHOICES:await m.reply(f"❌ Неверный выбор! Варианты: камень, ножницы, бумага");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  bot_choice=random.choice(["камень","ножницы","бумага"]);result=rps_result(choice,bot_choice);old_bal=bal
  if result=="win":
   win_amt=bet*2;user_balances[uid]=user_balances.get(uid,0)+win_amt
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
   ...
   if uid not in user_stats:user_stats[uid]={"played":0,"won":0,"total_bet":0,"total_win":0,"rps_wins":0}
   user_stats[uid]["played"]=user_stats[uid].get("played",0)+1
   user_stats[uid]["won"]=user_stats[uid].get("won",0)+1
   ...
   user_balances[uid]=bal-bet;user_balances[uid]=user_balances.get(uid,0)+bet
 ...
 if text.lower().startswith("спорт "):
  if len(parts)!=3:await m.reply("❌ Пример: спорт 500 1\n1 - победа хозяев, X - ничья, 2 - победа гостей");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: спорт 500 1");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bet_type=parts[2].upper()
  if bet_type not in["1","X","2"]:await m.reply("❌ Неверный тип ставки! Используй 1, X или 2");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  event=get_sport_event()
  if bet_type=="1":coef=event["coef1"]
  elif bet_type=="X":coef=event["coefX"]
  else:coef=event["coef2"]
  win=random.choice([True,False]);old_bal=bal
  if win:
   win_amt=int(bet*coef);user_balances[uid]=user_balances.get(uid,0)+win_amt
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
   await m.reply(f"⚽ <b>СПОРТИВНАЯ СТАВКА</b>\n\n{event['name']}\nСтавка: {bet_type} (x{coef})\n\n✅ ВЫ ВЫИГРАЛИ!\n💰 +{format_amount(win_amt)} GOLD",parse_mode="HTML")
   if uid not in user_stats:user_stats[uid]={"played":0,"won":0,"total_bet":0,"total_win":0,"sport_wins":0}
   user_stats[uid]["played"]=user_stats[uid].get("played",0)+1
   user_stats[uid]["won"]=user_stats[uid].get("won",0)+1
 ...
 if text.lower().startswith("кости "):
  if len(parts)!=3:await m.reply("❌ Пример: кости 500 на 7\nВарианты: на [2-12], на дубль, на чёт, на нечёт, на больше, на меньше");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: кости 500 на 7");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bet_type=parts[2].lower()
  if bet_type not in VALID_DICE_BETS:await m.reply(f"❌ Неверный тип ставки!");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  old_bal=bal;user_balances[uid]=bal-bet
  d1,d2=roll_dice();mult=get_dice_multiplier(bet_type,d1,d2)
  if mult>0:
   win=bet*mult;user_balances[uid]=user_balances.get(uid,0)+win
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
 ...
 if text.lower().startswith("слот "):
  if len(parts)!=2:await m.reply("❌ Пример: слот 100");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: слот 100");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  old_bal=bal;user_balances[uid]=bal-bet
  symbols=spin_slot();mult=get_slot_win(symbols)
  if uid not in user_stats:user_stats[uid]={"played":0,"won":0,"total_bet":0,"total_win":0,"slot_jackpot":0}
  if symbols==["💎","💎","💎"]:user_stats[uid]["slot_jackpot"]=user_stats[uid].get("slot_jackpot",0)+1
  if mult>0:
   win=bet*mult;user_balances[uid]=user_balances.get(uid,0)+win
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
 ...
 if text.lower().startswith("колесо "):
  if len(parts)!=2:await m.reply("❌ Пример: колесо 100");return
  try:bet=int(parts[1])
  except:await m.reply("❌ Пример: колесо 100");return
  if bet<100:await m.reply("❌ Минимальная ставка 100 GOLD");return
  bal=user_balances.get(uid,0)
  if bet>bal:await m.reply(f"❌ Недостаточно GOLD, баланс: {format_amount(bal)} GOLD");return
  old_bal=bal;user_balances[uid]=bal-bet
  sector=spin_wheel();mult=sector["mult"]
  if uid not in user_stats:user_stats[uid]={"played":0,"won":0,"total_bet":0,"total_win":0,"max_fortune":0}
  if mult>user_stats[uid].get("max_fortune",0):user_stats[uid]["max_fortune"]=mult
  if mult>0:
   win=bet*mult;user_balances[uid]=user_balances.get(uid,0)+win
   new_rank=check_rank_up(uid,old_bal,user_balances[uid])
   if new_rank:await send_rank_up_notify(uid,new_rank,m)
 ...
 if len(parts)>=2:
  ...
  total=amt*len(bets);bal=user_balances.get(uid,0)
  if total>bal:return await m.reply(f"❌ Нужно {format_amount(total)} GOLD")
  update_quest_progress(uid,"make_bet");update_quest_progress(uid,"make_5_bets")
  user_balances[uid]=bal-total;save_data()
  acc=[]
  for b in bets:
   if not b:continue
 ...
 if text.lower()=="го":
  ...
  game_in_progress=True;last_game_time=time.time()
  ...
  try:
   ...
   for b in pending_bets:
    ...
    if check_win(raw,num,color):
     mult=get_multiplier(raw);win_amt=amt*mult
    ...
    for badge in new_badges:await send_badge_notify(b["user_id"],badge,m)
   save_data()
   ...
   pending_bets.clear();game_in_progress=False;last_game_time=int(time.time())
  return
 ...
 if m.reply_to_message and text.lower().strip()=='дать всё':
  ...
  user_balances[uid]=0;user_balances[tid]=user_balances.get(tid,0)+bal
  save_data()
...
async def bj_cb(call):
 ...
 if action=="hit":
  ...
  if pv>21:
   g["active"]=False;del blackjack_games[uid]
   if uid not in user_stats:user_stats[uid]={"played":0,"won":0,"total_bet":0,"total_win":0}
   user_stats[uid]["played"]=user_stats[uid].get("played",0)+1
   ...
   new_badges=check_badges(uid)
   for badge in new_badges:await send_badge_notify(uid,badge,call.message)
   save_data()
  ...
  pv=hand_value(g["player_hand"]);dv=hand_value(g["dealer_hand"])
  win=0
  ...
  new_badges=check_badges(uid)
  for badge in new_badges:await send_badge_notify(uid,badge,call.message)
  del blackjack_games[uid];save_data()
...
async def mine_cb(call):
 ...
 if g["field"][r][c]=="💣":
  g["active"]=False;del mines_games[uid]
  update_quest_progress(uid,"play_3_games");update_quest_progress(uid,"play_10_games")
  daily_quests["win_3_streak"]["current_streak"][uid]=0
  new_achs=check_achievements(uid)
  for ach in new_achs:await send_achievement_notify(uid,ach,call.message)
