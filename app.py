...
def get_quests_status(uid):
    ...
    return "\n".join(lines)def get_main_keyboard_page1():
...
async def menu_cb(call):
    ...
    if act=="balance":
        ...
        if act in ["roulette","dice","slot","wheel"]:
            await call.message.edit_text(texts.get(act,"❓ Помощь"),parse_mode="HTML",reply_markup=get_main_keyboard_page1())
        else:
            await call.message.edit_text(texts.get(act,"❓ Помощь"),parse_mode="HTML",reply_markup=get_main_keyboard_page2())@dp.message_handler()
async def handle(m):
    ...
    if text.lower().startswith("слот "):
        if len(parts)!=2:
            await m.reply("❌ Пример: слот 100\nНикаких лишних слов после ставки!")
            return
        try:bet=int(parts[1])
        except:
            await m.reply("❌ Пример: слот 100")
        ...
        try:
            ...
            for b in pending_bets:
                ...
                if check_win(raw,num,color):
                    ...
                    new_rank=check_rank_up(b["user_id"], old_bal_bet, user_balances[b["user_i